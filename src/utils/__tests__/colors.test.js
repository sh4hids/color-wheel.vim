import { hexToRgb, rgbToHex, rgbTox255, hslToRgb } from '../colors';
...
describe('colors', () => {
  ...
  describe('hslToRgb', () => {
    const color1 = [300, 100, 50];
    const color2 = [258, 57, 50];
    const color3 = [258, 50];
    const expectedResult1 = [255, 0, 255];
    const expectedResult2 = [98, 55, 200];
    const expectedResult3 = new Error('Invalid hsl color');

    it(`${color1} should convert to ${JSON.stringify(
      expectedResult1
    )}`, async () => {
      expect(hslToRgb(color1[0], color1[1], color1[2])).toEqual(
        expectedResult1
      );
    });
    ...
    it(`${color2} should convert to ${JSON.stringify(
      ...
      expect(hslToRgb(color2[0], color2[1], color2[2])).toEqual(
        expectedResult2
      );
    });
    it(`${color3} should convert to '${expectedResult3}'`, async () => {
      try {
        hslToRgb(color3[0], color3[1], color3[2]);
      } catch (error) {
        expect(error).toEqual(expectedResult3);
      }
    });
